...
let _props = "padding", _demoElmId, _cssList, _cssProperties, 
_propertyOption, _outPutTag, _htmlView, _cssProp, _isClick, 
_dropDownCss, _cssOptions, _cssPropList, getActiveOpt, _newCssKeyVal, 
_cssKey, _cssVal, _cssValInpt, _getPropsVal, _inptBox, 
_getProps, strPredefind, preDefind, _upPreDefStr, newPreDef, cssPr, _setProp,
_getEditCssObj = {}, _updateVal = [], _initObjProp, _initObjVal,
 _getUptStyle, updateStyle,
_headTag = document.head || document.getElementsByTagName('head')[0],
_style = document.createElement('style'),
newArrObj, findCaps = /\W+|(?=[A-Z])|_/g, preDefindObj;
...
let onCssLoad = ()=>{
	...
	addCssProps(newArrObj["cssItems"], false);
...
function addCssProps(getCss, isCss){
	_setProp = "";
	if(isCss){
		...
		//Add css props obj
		_getEditCssObj[Object.keys(cssPr)[0]] = Object.values(cssPr)[0];
		_setProp = Object.keys(cssPr)[0];
		_getPropsVal = Object.values(cssPr)[0].split(" ");
	}
	...
	_newCssKeyVal.setAttribute("data-props", _setProp);
	_cssKey.setAttribute("class", "csskey");
	_cssVal.setAttribute("class", "cssval");

	_cssKey.innerText = _setProp.split(findCaps).join("-").toLowerCase()+" :";
	...
	for(let x = 0; x < _getPropsVal.length; x++){
		...
		for(let n = 0; n < strPredefind.length; n++){
			if(_getPropsVal[x] == strPredefind[n]){
				...
				_cssValSelect.setAttribute("class", "css-select"+" "+ _setProp);
				_cssValSelect.setAttribute("data-type", _setProp);
		...
		if(x != _notValOpt){
			...
			_cssValInpt.setAttribute("class", "css-inpt"+" "+ _setProp);
			// _cssValInpt.setAttribute("class", i);

			_cssValInpt.setAttribute("data-type", _setProp);
